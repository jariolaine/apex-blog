...
'--    Jari Laine 13.04.2022 - Bug fix to functi'))
);
...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(32897013199918411)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'ons is_integer, is_url and is_date_format error message handling',
...
'      ,p_value01      => p_post_id',
'    );',
'    -- raise no data found error if parameter p_post_id is nu'))
);
null;
wwv_flow_imp.component_end;
end;
...
'    if l_code_cnt = regexp_count( p_comment, ''\<\/code\>'', 1,'))
);
null;
wwv_flow_imp.component_end;
end;
